     ...
     if (source != mInputSource) {
         if ((source == AUDIO_SOURCE_DEFAULT) || (mMode != AudioSystem::MODE_IN_CALL)) {
             ...
             if (mMixerAlternate != NULL) {
                 LOGV("I have a second mixer, let's see if it has an input");
                     ...
                     if (ctl) {
                        ...
                        LOGV("mixer_ctl_select, Input Source, (USB)");
                     }
...
status_t AudioHardware::AudioStreamInALSA::open_l()
{
    ...
    TRACE_DRIVER_IN(DRV_PCM_OPEN)
    LOGV("Have alternate card: %d using %dHz rate",haveAlternateCard, config.rate);
